...
# Load environment variables (local .env or Render's env)
load_dotenv()
...
async def scalpingfirst(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "At TMTC, scalping is the foundation. Scalping teaches precision, discipline, and speed "
        "— all required skills before expanding to larger timeframes. Master scalping first to build "
...
async def the8020rule(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "Markets range 80% of the time and trend only 20% of the time. TMTC trades differently by "
        "focusing on sniper entries at extremes, not chasing breakouts into inefficient expansions."
    )
...
async def valuearea(ctx):
    ...
    await ctx.send(
        ...
        "Key levels: VAH (high), VAL (low), POC (point of control). TMTC uses the Value Area "
        "to find sniper deviations, not random trades inside it."
    )
...
async def sfp(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "An SFP (Swing Failure Pattern) occurs when price sweeps above a high or below a low "
        "to trigger liquidity, then sharply rejects. In TMTC, SFPs are critical entry confirmations."
    )
...
async def chochbos(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "CHOCH (Change of Character) marks the first sign of a directional shift — the first major "
        "break against previous trend. BOS (Break of Structure) confirms continuation after a CHOCH."
    )
...
async def steveshallway(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "Steve’s Hallway refers to limited scalping opportunities inside the Value Area. Hallway "
        "scalps must align with structure shifts (CHOCH) and require liquidity sweeps and fast "
        "confirmation. Only take hallway trades with strict stop control (<1%)."
    )
...
async def faileauction(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "A Failed Auction happens when price attempts to break outside a Value Area but fails to "
        "find acceptance, quickly snapping back inside. TMTC trades these failures as high-probability "
        "reversal setups after liquidity sweeps."
    )
...
async def wd40(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "WD40 Method requires Weekly, Daily, and 4H timeframe alignments before considering sniper "
        "trades. TMTC uses WD40 filtering to confirm directional bias and avoid random counter-trend entries."
    )
...
async def model1(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "Model 1 represents a full structure swing. After a CHOCH, price retraces back into the "
        "Value Area for a liquidity sweep and Golden Pocket retest before continuation. Full confirmation needed."
    )
...
async def model2(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "Model 2 is the Hallway Scalping model. Quick structure shifts (CHOCH) and liquidity sweeps occur "
        "inside the Value Area, offering fast sniper entries. Must be low-risk, fast-confirmation setups."
    )
...
async def adb(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "ADB (Ascending Double Bottom) happens when price sweeps a low, prints a higher low, and breaks "
        "structure upwards. TMTC uses ADB patterns for precision sniper entries after liquidity sweeps."
    )
...
async def cancel(ctx):
    ...
    await ctx.send(
        ...
        "and manage your subscription directly. TMTC cannot cancel memberships on your behalf."
    )
...
async def riskpolicy(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "TMTC Risk Management: Always risk 0.25–0.5% per trade maximum. Preserve your capital. Mastery comes "
        "through longevity, not oversized risk."
    )
...
async def coachinginfo(ctx):
    if not has_militia_role(ctx):
        ...
        return
    await ctx.send(
        "Private coaching requires advance payment. Sessions expire in 90 days. Contact @TMTC Support for onboarding."
    )
